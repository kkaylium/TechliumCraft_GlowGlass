package kkaylium.TechliumCraft.lib;

public class BlockIds {
	
	public static int GLOW_DIRT_defaultId = 252;
	public static int GLOW_STONE_defaultId = 253;
	
	public static int GLOW_BLOCK_defaultId = 500;
	public static int GLOW_PLANKS_defaultId = 501;
	public static int GLOW_LOG_defaultId = 502;
	public static int GLOW_LEAVES_defaultId = 503;
	public static int GLOW_ORE_defaultId = 504;
	public static int GLOW_GLASS_defaultId = 505;
	public static int GLOW_FENCE_defaultId = 506;
	public static int GLOW_GATE_defaultId = 507;
	public static int GLOW_DOOR_defaultId = 508;
	public static int GLOW_SAPLING_defaultId = 509;
	public static int GLOW_PORTAL_defaultId = 510;
	
	public static int INFUSERL1_defaultId = 511;
	
	public static int SERUM_DIRTY_STILL_defaultId = 521;
	public static int SERUM_DIRTY_FLOWING_defaultId = 522;
	
	public static int GLOW_DIRT_ID;
	public static int GLOW_STONE_ID;
	
	public static int GLOW_BLOCK_ID;
	public static int GLOW_PLANKS_ID;
	public static int GLOW_LOG_ID;
	public static int GLOW_LEAVES_ID;
	public static int GLOW_ORE_ID;
	public static int GLOW_GLASS_ID;
	public static int GLOW_FENCE_ID;
	public static int GLOW_GATE_ID;
	public static int GLOW_DOOR_ID;
	public static int GLOW_SAPLING_ID;
	public static int GLOW_PORTAL_ID;
	
	public static int INFUSERL1_ID;
	
	public static int SERUM_DIRTY_STILL_ID;
	public static int SERUM_DIRTY_FLOWING_ID;

}
