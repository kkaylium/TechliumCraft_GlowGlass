...
public class OverworldGenGlowTree extends WorldGenerator{
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
        ...
        if (par4 >= 1 && par4 + l + 1 <= 256)
        {
                        ...
                        for (int l1 = par3 - b0; l1 <= par3 + b0 && flag; ++l1)
                        {
                                for (j1 = par5 - b0; j1 <= par5 + b0 && flag; ++j1)
                                {
                                        if (i1 >= 0 && i1 < 256)
                                        {
                                                ...
                                                /** Custom grass block **/
                                                Block block = BlocksInit.glowDirt;
                                                if (k1 != 0 && !block.isLeaves(par1World, l1, i1, j1)

                                                                /** Custom grass block **/
                                                                && k1 != BlocksInit.glowDirt.blockID
                ...
                else
                {
                        ...
                        /** Custom grass block **/
                        Block soil = BlocksInit.glowDirt;

                        /** Custom Sapling block **/
                        boolean isSoil = (soil != null && soil.canSustainPlant(par1World, par3, par4 - 1, par5, ForgeDirection.UP, (BlockGlowSapling)BlocksInit.glowSapling));
                                        ...
                                        for (j2 = par3 - i2; j2 <= par3 + i2; ++j2)
                                        {
                                                ...
                                                for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2)
                                                {
                                                        ...
                                                        if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || par2Random.nextInt(2) != 0 && k1 != 0)
                                                        {
                                                                ...
                                                                {
                                                                        /** Custom leaf block **/
                                                                        this.setBlockAndMetadata(par1World, j2, j1, l2, BlocksInit.glowLeaves.blockID, this.metaLeaves);
                                                                }
                                                        }
                                                }
                                        }
