...
public class MapGenGlowCaves extends MapGenBase{
    ...
    private boolean isTopBlock(byte[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? data[index] == (byte)BlocksInit.glowDirt.blockID : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        int top    = (isExceptionBiome(biome) ? (byte)BlocksInit.glowDirt.blockID : biome.topBlock);
        int filler = (isExceptionBiome(biome) ? (byte)BlocksInit.glowDirt.blockID  : biome.fillerBlock);
        int block  = data[index];

        if (block == (byte)BlocksInit.glowStone.blockID || block == filler || block == top)
        {
            if (y < 10)
            {
