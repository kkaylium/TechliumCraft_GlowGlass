...
import kkaylium.TechliumCraft.gen.glowLand.gen.MapGenGlowCaves;
...
public class ChunkProviderGlowLand implements IChunkProvider{
	...
	private MapGenBase caveGenerator = new MapGenGlowCaves();
	...
	public ChunkProviderGlowLand(World par1World, long par2, boolean par4) {
			...
			for (int var9 = 0; var9 < 16; var9++) {
				...
				for (int var16 = 127; var16 >= 0; var16--) {
					...
					if (var16 <= 0 + this.rand.nextInt(5)) {
						 ...
						 **/
						else if (var18 != BlocksInit.glowStone.blockID) {
							if (var13 == -1) {
								if (var12 == 0) {
									var14 = 0;

									/** change to custom dirt **/
									var15 = (byte) BlocksInit.glowDirt.blockID;//
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		for (int c = 60; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == BlocksInit.glowDirt.blockID)) {
				new OverworldGenGlowTree(true, 6 + rand.nextInt(8), 0, 0, false)
						.generate(worldObj, rand, j2, l3, j5);
			}
		}
		for (int c = 50; c > 0; c--) {
			int j2 = var4 + rand.nextInt(16) + 8;
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == BlocksInit.glowDirt.blockID)) {
				new OverworldGenGlowTree(true, 4 + rand.nextInt(8), 0, 0, false)
						.generate(worldObj, rand, j2, l3, j5);
			}
		}
		for (int c = 50; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == BlocksInit.glowDirt.blockID)) {
				new OverworldGenGlowTree(true, 8 + rand.nextInt(8), 0, 0, false)
						.generate(worldObj, rand, j2, l3, j5);
			}
		}
		for (int c = 50; c > 0; c--) {
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == BlocksInit.glowDirt.blockID)) {
				new OverworldGenGlowTree(true, 10 + rand.nextInt(8), 0, 0,
						false).generate(worldObj, rand, j2, l3, j5);
			}
		}
		for (int c = 50; c > 0; c--) {
			int j2 = var4 + rand.nextInt(16) + 8;
			...
			if ((worldObj.getBlockId(j2, l3, j5) == 0)
					&& (worldObj.getBlockId(j2, l3 - 1, j5) == BlocksInit.glowDirt.blockID)) {
				new OverworldGenGlowTree(true, 15 + rand.nextInt(4), 0, 0,
						false).generate(worldObj, rand, j2, l3, j5);
			}
		}
		for (int c = 60; c > 0; c--) {
